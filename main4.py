...
print(most_influential_columns_for_fetal_health)
...
print(most_influential_columns_for_maternal_health)
...
if selected == 'Pregnancy Risk Prediction':
    ...
    if st.button('Predict Pregnancy Risk'):
        try:
            ...
            predicted_risk = maternal_model.predict([[age, diastolicBP, BS, bodyTemp, heartRate]])[0]
            if predicted_risk == 0:
                risk_level = "Low Risk"
    ...
    fields = [
        ...
        'Severe Decelerations', 'Prolongued Decelerations', 'Abnormal Short Term Variability',
        'Mean Value Of Short Term Variability', 'Percentage Of Time With ALTV', 'Mean Value Long Term Variability',
        'Histogram Width', 'Histogram Min', 'Histogram Max', 'Histogram Number Of Peaks', 'Histogram Number Of Zeroes',
    ...
    if st.button('Predict Fetal Health'):
        try:
            predicted_health = fetal_model.predict([fetal_features])[0]
